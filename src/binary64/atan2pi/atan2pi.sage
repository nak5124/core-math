...
def table_err_poly():
   ...
   for i in range(0,64):
      if i == 0: # special case
         err = 4.55142303056794e-20
      else:
         p, err = sollya_approx((i+0.5)/64,1/128.,7,1)
         err = err_poly(p,i,-1./128,1./128,err)
      # the following takes into account the error while multiplying by 1/pi
      err = (1+R(err))*(1+R(2^-107.871))*(1+R(2^-98.616))-1
      # the following takes into account the maximal error when adjusting
      # the quadrant
      err = err + 2^-98.912
      if i==0:
         s = get_hex(R9(err))
