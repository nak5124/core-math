...
# statall("out")
# [((8183583624766078, -79), (7074237752028441, -52))]
def statall(f):
   f = open(f,"r")
   l = []
   while true:
...
def reduce_fast_case1_aux(xmin,xmax,bound,check_sin):
   ...
   for i0 in [imin..imax]:
      i = i0
      hmin = max(h0.lower(), RR(i/2^11))
      hmax = min(h0.upper(), RR((i+1)/2^11).nextbelow())
      hmin = hmin - i/2^11
      hmax = hmax - i/2^11
      l = l0
      hh = RIF(hmin,hmax)
...
# reduce_fast_case2_all(bound=2^-71.,check_sin=true)
# (4.10204912823919e-22, -71.0460733171467)
# reduce_fast_case2_all(bound=2^-71.,check_sin=false)
def reduce_fast_case2_all(bound=2^-71.,check_sin=true):
   maxerr = 0
   for e in [1025..1074]:
      err, _ = reduce_fast_case2(e,bound=bound,check_sin=check_sin)
      print (e, err, log(err)/log(2.))
      maxerr = max(maxerr, err)
   err = maxerr
   return err, log(err)/log(2.)

# bound error relerr1 of reduce_fast relative to sin2pi(R)
# in the case 0x1.921fb54442d17p+2 < x < 2^52
# | sin2pi(R) - sin|x| | < relerr1 * |sin2pi(R)|
# reduce_fast_case2(1025,bound=2^-72.)
# (2.69614998011179e-22, -71.6515173352361)
def reduce_fast_case2(e,bound=2^-71.,check_sin=true):
   assert 1025 <= e <= 1074, "1025 <= e <= 1074"
   if e==1025:
      mmin = 7074237752028439
   else:
      mmin = 2^52
   mmax = 2^53
   # x = m/2^53 * 2^(e-1022)
   err = reduce_fast_case2_aux(e,mmin,mmax,bound,check_sin)
   return err, log(err)/log(2.)

def reduce_fast_case2_aux(e0,mmin,mmax,bound,check_sin):
   e = e0
   # 1/(2*pi) ~ T[0]/2^64 + T[1]/2^128 + ... + T[i]/2^((i+1)*64) + ...
   # where |T[0]/2^64 + T[1]/2^128 - 1/(2pi)| < 2^-130.22
   xmax = mmax/2^53 * 2^(e-1022)
   err1 = 2^-130.22*xmax # error by ignoring T[2], T[3], ...
   T0 = 0x28be60db9391054a
   T1 = 0x7f09d5f47d4d3770
   e = 1075 - e
   cmin = mmin*(2^64*T0+T1)
   cmax = mmax*(2^64*T0+T1)
   assert 1 <= e <= 50, "1 <= e <= 50"
   cmin = floor(cmin/2^e)
   cmax = floor(cmax/2^e)
   err2 = 2^-128. # error when truncating cmin,cmax above
   qmin = cmin//2^128
   qmax = cmax//2^128
   if qmin!=qmax:
      cmin = 0
      ...
      cmax = cmax % (2^128)
   # set_dd (h, l, c[1], c[0])
   Hmin = floor(cmin/2^128*2^53)/2^53
   Lmin = floor((cmin/2^128-Hmin)*2^106)/2^106
   Hmax = floor(cmax/2^128*2^53)/2^53
   Lmax = floor((cmax/2^128-Hmax)*2^106)/2^106
   err3 = 2^-106. # truncation error in set_dd
   err = err1+err2+err3
   imin = floor(Hmin*2^11)
   imax = floor(Hmax*2^11)
   maxerr = 0
   assert 0 <= imin <= imax < 2^11, "0 <= imin <= imax < 2^11"
   for i0 in [imin..imax]:
      i = i0
      hmin = max(Hmin,i/2^11)
      hmax = min(Hmax,(i+1)/2^11-2^-53)
      hmin = hmin - i/2^11
      hmax = hmax - i/2^11
      l = RIF(0,Lmax)
      hh = RIF(hmin,hmax)
   ...
   if mmin==mmax:
      return maxerr
   # split [mmin,mmax] in two
   mmid = (mmin+mmax)//2
   e1 = reduce_fast_case2_aux(e0,mmin,mmid,bound,check_sin)
   e2 = reduce_fast_case2_aux(e0,mmid+1,mmax,bound,check_sin)
   return max(e1,e2)

# reduce_fast_case3_all(bound=2^-71.,check_sin=true)
# (1.55694665837632e-19, -62.4779142849132)
def reduce_fast_case3_all(bound=2^-71.,check_sin=true):
   maxerr = 0
   for e in [1075..2046]:
      err, _ = reduce_fast_case3(e,bound=bound,check_sin=check_sin)
      print (e, err, log(err)/log(2.))
      maxerr = max(maxerr, err)
   err = maxerr
   return err, log(err)/log(2.)
...
# bound error relerr1 of reduce_fast relative to sin2pi(R)
# in the case 2^52 <= x < 2^1024
# | sin2pi(R) - sin|x| | < relerr1 * |sin2pi(R)|
# 1393 1.40001727760917e-20 -65.9531172661718
def reduce_fast_case3(e,bound=2^-71.,check_sin=true):
   assert 1075 <= e <= 2046, "1075 <= e <= 2046"
   mmin = 2^52
   mmax = 2^53
   # x = m/2^53 * 2^(e-1022)
   err = reduce_fast_case3_aux(e,mmin,mmax,bound,check_sin)
   return err, log(err)/log(2.)
...
def reduce_fast_case3_aux(e0,mmin,mmax,bound,check_sin):
   e = e0
   i = (e - 1138 + 63) // 64
   cmin = mmin*(T[i+2]+2^64*T[i+1]+2^128*T[i])
   cmax = mmax*(T[i+2]+2^64*T[i+1]+2^128*T[i])
   err1 = mmax*(T[i+3]+1)/2^64. # relative to cmax
   e = 1139 + i*2^6 - e
   assert 1 <= e <= 64, "1 <= e <= 64"
   cmin = floor(cmin/2^e)
   cmax = floor(cmax/2^e)
   err1 = err1/2^e/2^128.
   err2 = 2^-128. # error when truncating cmin,cmax above
   ...
   # split [mmin,mmax] in two
   mmid = (mmin+mmax)//2
   e1 = reduce_fast_case3_aux(e0,mmin,mmid,bound,check_sin)
   e2 = reduce_fast_case3_aux(e0,mmid+1,mmax,bound,check_sin)
   return max(e1,e2)
