...
def bacsel_doit(e):
   ...
   x0 = R(2^(e-1))
   x1 = R(2^e)
   y0 = numerical_approx(atan(x0.exact_rational())/pi,200)
   y1 = numerical_approx(atan(R(x1).nextbelow().exact_rational())/pi,200)
   if y1<2^-1022: # all output in subnormal range
      ...
      while a.nextabove()!=b:
         c = (a+b)/2
         y = Rz(numerical_approx(atan(c.exact_rational())/pi,200))
         if y<2^-1022:
            a = c
         else:
            b = c
   ...
   if x0<x1: # subnormal output in x0,x1
      y0 = Rz(numerical_approx(atan(x0.exact_rational())/pi,200))
      y1 = Rz(numerical_approx(atan(x1.nextbelow().exact_rational())/pi,200))
      if y0.ulp()!=y1.ulp():
         a = x0
         b = x1
         while a.nextabove()!=b:
      ...
      # process [x0,xm)
      xm = xm.nextbelow()
      ym = Rz(numerical_approx(atan(xm.exact_rational())/pi,200))
      u0 = floor(y0*2^1074)
      um = floor(ym*2^1074)
      nn = u0.nbits()
      ...
      if xm<x1:
         ym = Rz(numerical_approx(atan(xm.exact_rational())/pi,200))
         x1 = x1.nextbelow()
         y1 = Rz(numerical_approx(atan(x1.exact_rational())/pi,200))
         um = floor(ym*2^1074)
         u1 = floor(y1*2^1074)
         nn = um.nbits()
...
# statall("out")
# [((4, -1074), (9007199254740992, -1073)), ((4503599627370496, -106), (5734161139222659, 1))]
def statall(f):
   f = open(f,"r")
   l = []
   while true:
